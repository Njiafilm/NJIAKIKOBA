...
def dashboard():
    conn = db()
    user = conn.execute(
        "SELECT full_name, phone, payment_phone, payment_network, payout_method, bank_code, bank_account, bank_account_name, email, savings, loan_balance FROM users WHERE id=?",
        (session["user_id"],)
    ).fetchone()
...
def register_leaders():
    if request.method == "POST":
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO users (full_name, phone, password, role, id_type, id_number_hash) "
                "VALUES (?, ?, ?, 'leader', ?, ?)",
