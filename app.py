...
def developer_dashboard():
    ...
    settings = conn.execute(
        "SELECT adsense_publisher_id, play_store_url, appstore_url, group_name, registration_code, member_cap, "
        "settlement_phone, settlement_bank_name, settlement_bank_account, "
        "partner_name, partner_contact, partner_account_details, cycle_months, cycle_started_at "
        "FROM group_info LIMIT 1"
    ).fetchone()
    counts = {
    ...
    last_cycle = conn.execute(
        "SELECT total_group_savings, commission_amount, cycle_months, created_at FROM cycle_settlements ORDER BY id DESC LIMIT 1"
    ).fetchone()
    ...
    return render_template(
        ...
        integrations=integrations, group_summary=group_summary, last_cycle=last_cycle
    )
...
def end_cycle():
    conn = db()
    total_savings = conn.execute("SELECT COALESCE(SUM(savings),0) FROM users").fetchone()[0]
    group = conn.execute("SELECT cycle_months FROM group_info LIMIT 1").fetchone()
    cycle_months = group["cycle_months"] if group and group["cycle_months"] else 12
    commission = round(total_savings * 0.02, 2)
    conn.execute(
        "INSERT INTO cycle_settlements (total_group_savings, commission_amount, cycle_months) VALUES (?, ?, ?)",
        (total_savings, commission, cycle_months)
    )
    conn.execute("UPDATE group_info SET cycle_started_at=CURRENT_TIMESTAMP")
    conn.execute(
...
def developer_settings_update():
    ...
    conn.execute(
        ...
        + (", member_cap=?" if member_cap is not None else ""),
        (adsense_id or None, play_store or None, appstore or None, group_name or "NJIAKIKOBA",
         ...
         partner_name or None, partner_contact or None, partner_account_details or None, int(cycle_months_raw))
        + ((member_cap,) if member_cap is not None else ())
    )
    conn.commit()
    conn.close()
...
def create_payment():
    ...
    is_first_deposit = tx_type == "savings" and conn.execute(
        "SELECT COUNT(*) FROM transactions WHERE user_id=? AND tx_type='savings' AND status='paid'",
        (session["user_id"],)
    ).fetchone()[0] == 0
    conn.close()
